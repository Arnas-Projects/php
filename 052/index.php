<?php

$backgroundColor = '#444';
$textColor = 'white';
echo "<body style='background-color: $backgroundColor; color: $textColor; font-size: 24px; line-height: 1.5;'>";


//////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////

// CIKLAI

echo 'FOR <br><br>';

for ($i = 0; $i < 5; $i++) {
    echo $i . '<br>';
};


echo '<br><hr>WHILE<br><br>';

$i = 0;

while ($i < 5) {
    echo $i . '<br>';
    $i++;
};


echo '<br><hr>DO WHILE<br><br>';

$i = 0;

do {
    echo $i . '<br>';
    $i++;
} while ($i < 5);


echo '<br>FOR in FOR <br><br>';

for ($i = 0; $i < 5; $i++) {
    echo 'I: ' . $i . '<br>';

    for ($j = 0; $j < 5; $j++) {

        if ($j == 2) {
            // continue; // nutraukia mažo ciklo iteraciją
            continue 2; // nutraukiam mažą ciklą ir didžiojo ciklo iteraciją
        }

        echo '----J: ' . $j . '<br>';
        // break; // mažo ciklo nutraukimas
        // break 2; // 2 --> nutraukia mažą ir tėvinį ciklą
    }

    // break ; // didelio ciklo nutraukimas

};

// JOKE Tikri PHP programeriai nenaudoja FOR ciklo :)


echo '<br><br><hr><br>'; 

//////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////
// MASYVAI

$masyvas1 = []; // tuščias masyvas
$masyvas2 = array(); // senovinis būdas, taip nerašyti, nes juoksis iš jūsų kodo

echo '<pre>';
echo '<br>ARRAY<hr><br>';

print_r($masyvas1);


echo '<br><hr><br>';


$masyvas3 = [4, 8, 'Bebras', 5 => 'Barsukas', 0];

// 5 yra ineksas => tada reikšmė

// sekantis elementas yra didžiausias indeksas +1

// 3 nedingo
$masyvas3[3] = 55;

print_r($masyvas3);

echo '<br><hr>';

$masyvas4 = [
    4,
    8 => 'Bebras',
    'Barsukas',
    'labas' => 55,
    'Briedis'
];

$masyvas4['labas'] = 107;

$masyvas4['14'] = 107; // jeigu indeksą galima paversti skaičiumi, tai taip ir bus
$masyvas4[14] = 107;

$masyvas4[] = 222; // push į masyvą, indeksas max indeksas + 1;

print_r($masyvas4);


echo '<br><hr>';


$zmogus = [
    'vardas' => 'Jeronimas',
    'auto' => 'Cadillac',
    'amzius' => 55
];

print_r($zmogus);


echo '<br><hr>';


// Reikia sukurti masyvą, kuriame būtų 10 reikšmių: 0, 2, 4, 6... 18.

$array1 = [];

for ($i = 0; $i < 10; $i++) {
    $array1[] = $i * 2;
};

print_r($array1);

//////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////        ======== FOR EACH ========        //////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////

echo '<br><hr>FOR EACH<br><br>';

foreach($zmogus as $reiksme){
    
    echo $reiksme . '<br>';
};



echo '<br><hr>FOR EACH<br><br>';

foreach($zmogus as $indeksas => $reiksme){
    
    echo $indeksas . ': ' . $reiksme . '<br>';
};



echo '<br><hr><br>';


$masyvas123 = range('Z', 'a'); // padaro masyvą

print_r($masyvas123);

foreach(range(1, 5) as $i) {
    echo $i . '<br>';
};


// pagal reikšmę ir pagal nuorodą

echo '<br>--------------<hr><br>';

$m1 = [1, 2, 3];
$m2 = $m1; // perdavimas pagal reikšmę

$m1[] = 4;

print_r($m1);
print_r($m2);

echo '<br>--------------<hr><br>';

$m3 = [1, 2, 3];
$m4 =& $m3; // perdavimas pagal reikšmę

$m3[] = 4;

print_r($m3);
print_r($m4);


echo '<br>--------------<hr><br>';


$colors = ['Red', 'Green', 'Blue', 'Yellow'];

foreach($colors as &$color){} // $color pagal nuorodą (by reference)

// reikėtų nepamiršti panaudotą nebereikalingą nuorodą "užmušti"

unset($color); // unset panaikina nuorodą

foreach($colors as $color) { // $color pagal reikšmę (by value)
    echo $color . '<br>';
};


echo '<br><hr><br>';


$fun = function($el) {
    return $el . ' ***';
};

$naujasZmogus = array_map($fun, $zmogus);

print_r($naujasZmogus);

