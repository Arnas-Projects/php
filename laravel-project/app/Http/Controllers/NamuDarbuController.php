...
class NamuDarbuController extends Controller
{
    ...
    public function formaStorage()
    {
        return view('skaiciai.storage');
    }

    public function formosStorageApdorojimas(Request $req)
    {
        $validated = $req->validate(
            [
                'skaicius1' => 'required|integer|min:0|max:99',
            ...
            [
                'skaicius1.required' => 'Skaičius yra privalomas',
                'skaicius1.integer' => 'Skaičius turi būti sveikasis skaičius',
                'skaicius1.min' => 'Skaičius turi būti ne mažesnis nei 0',
                'skaicius1.max' => 'Skaičius turi būti ne didesnis nei 99'
            ]
        );
        ...
        $oldRez[] = $sk1; // į masyvą įpushinam naują skaičių

        session(['rez2' => $oldRez]); // įrašome seną skaičių sąrašą su pridetu nauju skaičiumi, į sesiją

        return redirect()->route('rodymas-storage');
    }

    public function formosStorageRodymas()
    {
        $rezultatas = session('rez2', '');

        return view('skaiciai.storage_rezultatas', [
            'rezultatas' => $rezultatas
        ]);
    }

    public function formosStorageValymas(Request $req)
    {
